...
def get_convnet(args, pretrained=False):
    ...
    # VPT
        ...
        if "adapter" in args["model_name"]:
            from petl import vision_transformer_adapter, vit_MoE_adapter
            ...
            if name=="pretrained_vit_b16_224_adapter":
                model = vit_MoE_adapter.vit_base_patch16_224_adapter(num_classes=0,
                    global_pool=False, drop_path_rate=0.0, tuning_config=tuning_config)
                model.out_dim=768
            elif name=="pretrained_vit_b16_224_in21k_adapter":
                model = vit_MoE_adapter.vit_base_patch16_224_in21k_adapter(num_classes=0,
                    global_pool=False, drop_path_rate=0.0, tuning_config=tuning_config)
                model.out_dim=768
            else:
